...
export async function importarFuncionarios(linhas: LinhaImportacao[]): Promise<ResultadoImportacao> {
  ...
  for (let i = 0; i < linhas.length; i++) {
    ...
    try {
      ...
      const mesaRegistro =
        (await mesasRepository.buscarPorNome(linha.mesa)) ??
        (await mesasRepository.criar({ nome: linha.mesa }));
      const funcionario = await funcionariosRepository.cadastrarOuAtualizarPorMatricula({
        matricula: linha.matricula,
        nome: linha.nome
      });
      await lugaresRepository.atribuirFuncionarioAoLugar(mesaRegistro.id, linha.posicao, funcionario.id);
      if (existente) {
