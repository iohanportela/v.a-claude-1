...
import { funcionariosRepository, mesasRepository, lugaresRepository } from '@database/index';
...
import type { Funcionario, NovoFuncionario } from '@domain/domain';
...
export function CadastroPage(): JSX.Element {
  ...
  async function salvar(dados: NovoFuncionario): Promise<void> {
    ...
    if (!mesaSelecionada || posicaoSelecionada === null) {
      mostrarToast('Selecione uma mesa e posição antes de cadastrar.', 'erro');
      return;
    }

    const funcionario = await funcionariosRepository.criar(dados);
    const mesa = await mesasRepository.buscarPorNome(mesaSelecionada);
    if (!mesa) {
      mostrarToast('Mesa não encontrada.', 'erro');
      return;
    }

    await lugaresRepository.atribuirFuncionarioAoLugar(mesa.id, posicaoSelecionada, funcionario.id);
    mostrarToast('Funcionário cadastrado e associado à posição.', 'sucesso');
    fecharModal();
  }
  ...
  async function criarNovaMesa(): Promise<void> {
    const nome = novaMesaTexto.trim();
    if (!nome) return;
    try {
      await mesasRepository.criar({ nome });
      setMesaSelecionada(nome);
      setNovaMesaTexto('');
      navigate(`/cadastro/${encodeURIComponent(nome)}`, { replace: true });
      mostrarToast(`Mesa ${nome} criada com 24 lugares vazios.`, 'sucesso');
    } catch (erro) {
      mostrarToast(erro instanceof Error ? erro.message : 'Falha ao criar a mesa.', 'erro');
    }
  }
