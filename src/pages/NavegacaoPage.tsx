...
export function NavegacaoPage(): JSX.Element {
  ...
  const [localizacao, setLocalizacao] = useState<{ mesa: string; posicao: number } | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      const local = await funcionariosRepository.buscarLocalizacao(funcionario.id);
      if (!cancelado) {
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="border-t border-base-800 bg-base-900 px-4 py-4">
        ...
        <div className="mt-1 grid grid-cols-2 gap-x-4 gap-y-1 text-sm text-base-300">
          ...
          <span>Mesa: {localizacao?.mesa ?? leituraAtual.mesa}</span>
          <span>Posição: {localizacao?.posicao ?? '—'}</span>
        </div>
