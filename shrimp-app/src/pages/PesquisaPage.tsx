...
export function PesquisaPage(): JSX.Element {
  ...
  async function abrirNaNavegacao(funcionarioId: string, mesa: string): Promise<void> {
      ...
      <div className="flex flex-col gap-2">
        {resultados.map((funcionario) => (
          <button
            key={funcionario.id}
            type="button"
            onClick={() => void abrirNaNavegacao(funcionario.id, funcionario.mesa)}
          ...
          >
            <span className="flex flex-col">
              ...
              <span className="text-sm text-base-400">
                Mat. {funcionario.matricula} · {funcionario.mesa} · Posição {funcionario.posicao}
              </span>
            </span>
